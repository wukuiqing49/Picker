...
import com.cnlive.media.ui.adapter.MediaAdapter;
import com.cnlive.media.ui.adapter.OnItemClickListener;
import com.cnlive.media.ui.widget.SpacingDecoration;
...
public class MediaPickerFragmentView implements MvpView {
    ...
    //处理数据选中的状态
    private void processSelectMedias(MediaActivity mActivity, boolean isSelect, Media media, int position) {
        if (isCheckMediaSupport(mActivity, media)) {
            MediaSelectStateObservable.getInstance().selectStateUpdateMedia(media);
            media.setSelect(!isSelect);
            mFragment.mMediaAdapter.notifyItemChanged(position, media);
        }
    }
