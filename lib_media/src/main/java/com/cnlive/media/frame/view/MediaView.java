...
import com.cnlive.media.ui.activity.MediaActivity;
import com.cnlive.media.ui.activity.RecordActivity;
import com.cnlive.media.ui.adapter.FolderAdapter;
import com.cnlive.media.ui.adapter.PreviewBottomMediaAdapter;
import com.cnlive.media.ui.fragment.MediaPickerFragment;
import com.cnlive.media.ui.fragment.MediaPreviewFragment;
import com.cnlive.media.utils.AlertDialogUtils;
import com.cnlive.media.utils.AndroidQUtil;
import com.cnlive.media.utils.FileTypeUtil;
import com.cnlive.media.utils.MediaStringUtils;
import com.cnlive.media.utils.PermissionChecker;
import com.cnlive.media.utils.ScreenUtils;
...
import com.cnlive.media.utils.observable.SelectStateUpdateObservable;
...
//        //初始化七牛录制
        PLShortVideoEnv.init(mActivity.getApplicationContext());
    ...
    public void showPrewBottom(Media mMedia) {
        ...
        if (mActivity.selects.indexOf(mMedia) < 0) {
            ...
            previewBottomAdapter = new PreviewBottomMediaAdapter(mActivity, mActivity.selects.indexOf(mMedia), mActivity.isPre);
        }
    ...
    public boolean isCheckMediaSupport(Media media) {
        ...
        if (media.mediaType == 1 && mActivity.maxImageSize < media.size) {
            String size = com.cnlive.media.utils.FileUtils.fileSize(mActivity.maxImageSize);
        ...
        if (media.mediaType == 3 && mActivity.maxVideoSize < media.size) {
            String size = com.cnlive.media.utils.FileUtils.fileSize(mActivity.maxVideoSize);
            showMessage(mActivity.getResources().getString(R.string.media_msg_size_limit) + size);
            return false;
        }
        //文件格式
